...
### 变更
- **项目身份与发布元数据已统一为 Gateway-tools**：package、Tauri、Rust、仓库、问题反馈、主页与文档引用均指向 `superaddmin/Gateway-tools.git`。
- **界面系统新增紧凑型应用基线文档**：`docs/ui-audit.md` 固化 token、组件密度、响应式规则、focus-visible 行为与减少动效的设计约束，便于后续界面改动保持一致。
- **Dashboard 与导航密度已面向工具型使用收紧**：总览卡片改为更易扫读的紧凑状态面板，高频 hover 动效减少，900px 以下布局使用真实窄屏导航路径。

### 修复
- **全局 focus-visible 状态现覆盖主要交互控件**：按钮、输入框、下拉、工具栏操作与可点击卡片都具备一致的键盘焦点反馈。
- **文档已对齐当前 Gateway-tools 代码与发布流程**：文档内容反映项目新名称、当前仓库、紧凑 UI 规范与真实本地运行数据边界。
